...
ensembles = [1, 2, 3, 4, 5, 35];

for e = 1 : length(ensembles)

    n_ensemble = ensembles(e);
    disp(['ensemble ', num2str(n_ensemble)])
    ...
    for climate = 1 : 2
        ...
        ta_avg = zeros(length(lon), length(lat), length(plevels));
        for k = 1 : length(plevels)
            disp(['k = ', num2str(k), ' out of ', num2str(length(plevels)), ' levels'])
            temp_ta = zeros(length(lon), length(lat), length(years));
            for f = 1 : length(years)
                disp(['year = ', num2str(years(f))])
                ta_filename = [input_path, 'ta_', string_1, '_', num2str(years(f)), '.nc'];
                start = [lon_indices(1), lat_indices(1), k, 1];
                count = [length(lon_indices), length(lat_indices), 1, Inf];
                temp = squeeze(ncread(ta_filename, 'ta_plevel', start, count));
                temp_ta(:, :, f) = mean(temp, 3, 'omitnan');
            end
            ta_avg(:, :, k) = mean(temp_ta, 3, 'omitnan');
        end
        nc_filename = [output_path, 'ta_avg_', string_1, '_', string_2, '.nc'];
        varname = 'ta_avg';
        writeNetCDF_omega(nc_filename, varname, ta_avg, lat, lon, plevels);
    end
