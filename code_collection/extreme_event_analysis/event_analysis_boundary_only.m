...
function event_analysis_boundary_only(box_min, box_max, p_min, p_max, lat_series_original, lon_series_original, lat_indices, lon_indices, ...
        ...
        for ii = 1 : length(lon_indices)
            ...
            if ~isempty(days_{jj, ii})
                ...
                for m = 1 : length(days_{jj, ii})
                    ...
                    if strfind(pwd_str, 'CESM')
                        [T, ~, ~, ~, ~, omega, omega_b, tag] = event_read_Wrapper_uava(computation_y, computation_x, length(level_indices), ...
                                event_year, input_path, event_latspan, event_lonspan, event_day, level_indices, string_1, JJA_DJF);
                    elseif strfind(pwd_str, 'GFDL')
                        %[T, ~, ~, ~, ~, omega, omega_b, tag] = event_read_Wrapper_uava_GFDL(computation_y, computation_x, length(level_indices), ...
                        %        event_year, input_path, event_latspan, event_lonspan, event_day, level_indices, string_1);
                        [T, omega, omega_b, tag] = event_read_Wrapper_boundary_GFDL(computation_y, computation_x, length(level_indices), ...
                                event_year, input_path, event_latspan, event_lonspan, event_day, level_indices, string_1);
                    end
                    if ~tag; continue; end;
                    ...
                    omega_b_temp = zeros(size(omega));
                    for t = 1 : size(omega, 4)
                        omega_b_temp(:, :, :, t) = omega_b;
                    end
                    omega_b_temp(:, :, 1, :) = omega(:, :, 1, :);
                    omega_b = omega_b_temp;
                    %{
                    ...
                    for t = 1 : length(event_timespan)
                        ...
                        [dtheta_dp_ma_avg(:, t), dtheta_dp_eff, theta_avg] = eff_stat_stab(level, T_avg(:, t));
                    ...
                    for t = 1 : length(event_timespan)
                        if ACCU_SIGMA
                            omega_QG(:, :, :, t) = SIP_diagnostic_v1_sigma(R, f0, length(lon), length(phi), length(level), phi, lambda, level, ...
                                                        sigma_accu(:, :, :, t), dphi, dlambda, rhs(:, :, :, t), omega_b(:, :, :, t), ACCU_BOUNDARY);
                        elseif exist('SIGMA_LOCAL') && SIGMA_LOCAL % Paul's idea, use local sigma_accu as zonal averaged sigma
                            ...
                            omega_QG(:, :, :, t) = SIP_diagnostic_v1(R, f0, length(lon), length(phi), length(level), phi, lambda, level, ...
                                                        sigma(:, t), dphi, dlambda, rhs(:, :, :, t), omega_b(:, :, :, t), ACCU_BOUNDARY);
                    else
                            omega_QG(:, :, :, t) = SIP_diagnostic_v1(R, f0, length(lon), length(phi), length(level), phi, lambda, level, ...
                                                        sigma(:, t), dphi, dlambda, rhs(:, :, :, t),  omega_b(:, :, :, t), ACCU_BOUNDARY);
                        end
