...
function [precip, precip_quantile] = percentile_grid_with_mask_month(n, lon_indices, lat_indices, files, percentage, MASK, h_max, ocean_only, p_max, months)
    ...
    for j = 1 : length(lat_indices)
        for i = 1 : length(lon_indices)
            precip_t = reshape(precip(i, j, :), nt, 1);
