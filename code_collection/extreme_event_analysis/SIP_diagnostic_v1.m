function omega = SIP_diagnostic_v1(r, f0, Ni, Nj, Nk, phi, lambda, level, sigma, dphi, dlambda, rhs, omega_b, ACCU_BOUNDARY)
    ...
    for k = 2 : Nkm
        ...
        for j = 2 : Njm
            %for i = 2 : Nim
                ...
                AW(ijk) = (1 / (r^2 * cos(phi(j))^2 * dlambda^2)) * sigma(k);
                AS(ijk) = (1 / (r^2 * dphi^2) + tan(phi(j)) / (2 * r^2 * dphi)) * sigma(k);
                AN(ijk) = (1 / (r^2 * dphi^2) - tan(phi(j)) / (2 * r^2 * dphi)) * sigma(k);
                AE(ijk) = AW(ijk);
