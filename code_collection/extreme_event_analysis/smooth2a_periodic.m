function matrixOut = smooth2a_periodic(matrixIn,Nr,Nc,periodic_dim)
...
% modification by ziweili
% add more data on periodic dimension, and subtract redundant data later

size_m = size(matrixIn);
if any(periodic_dim == 1)
    dim1 = [size_m(1) - Nr + 1 : size_m(1), 1 : size_m(1), 1 : Nr];
    dim1_reselect = Nr + 1 : size_m(1) + Nr;
else
    dim1 = 1 : size_m(1);
    dim1_reselect = dim1;
end
if any(periodic_dim == 2)
    dim2 = [size_m(2) - Nc + 1 : size_m(2), 1 : size_m(2), 1 : Nc];
    dim2_reselect = Nc + 1 : size_m(2) + Nc;
else
    dim2 = 1 : size_m(2);
    dim2_reselect = dim2;
end    
matrixIn = matrixIn(dim1, dim2);
...
% reduce redundant rows & cols

matrixOut = matrixOut(dim1_reselect, dim2_reselect);
